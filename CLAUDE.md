...
- `controller/api/ConcursoRestController` — `/api/loterias/{loteria}/...`: importacao (POST, 202/409), importacao/status, concursos (paged), concursos/{numero}, estatisticas, export (`formato=csv|json`).
- `controller/web/PaginasController` — JSP pages: `/` (home cards), `/loterias/{loteria}` (paged table), `.../concursos/{numero}` (detail), `.../dashboard` (Chart.js via CDN consuming the estatisticas endpoint).
...
- `repository/ConcursoRepository` — Spring Data Mongo repository over `ConcursoMongoDTO` (`@Document(collection = "resultados")`).
- `dto/` — `ConcursoDTO` (raw Caixa API shape), `ConcursoMongoDTO` + `FeaturesDTO` (persisted shape), `EstatisticasDTO`, `LinhaDataset` (export row).
